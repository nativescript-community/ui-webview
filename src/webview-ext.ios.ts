...
export class AWebView extends WebViewExtBase {
    ...
    public _loadUrl(src: string) {
        ...
        if (src.startsWith('file:///')) {
            ...
            if (Trace.isEnabled()) {
                Trace.write(
                    `WKWebViewWrapper.loadUrl("${src}") -> ios.loadRequest("${nsRequestWithUrl}"`,
                    NotaTraceCategory,
                    Trace.messageType.info
                );
            }
    ...
    public _loadData(content: string) {
        ...
        if (Trace.isEnabled()) {
            Trace.write(
                `WKWebViewWrapper.loadUrl(content) -> this.ios.loadHTMLStringBaseURL("${nsBaseUrl}")`,
                NotaTraceCategory,
                Trace.messageType.info
            );
        }
    ...
    public async autoLoadStyleSheetFile(resourceName: string, path: string, insertBefore?: boolean) {
        ...
        if (!filepath) {
            if (Trace.isEnabled()) {
                Trace.write(
                    `WKWebViewWrapper.autoLoadStyleSheetFile("${resourceName}", "${path}") - couldn't resolve filepath`,
                    NotaTraceCategory,
                    Trace.messageType.info
                );
            }
